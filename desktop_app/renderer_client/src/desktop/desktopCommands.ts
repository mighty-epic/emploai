...
const DESKTOP_NO_ACTIVE_SESSION_STATUS = 'Open or select a chat before using agent commands.';
...
const COMMAND_HELP: Record<string, string> = {
  ...
  schedule: 'Create an automation: `/schedule <name> <schedule> <prompt>`.',
  automations: 'List automations and scheduled event rules.',
  automation_run: 'Run an automation now: `/automation_run <automation_id>`.',
  automation_pause: 'Pause an automation: `/automation_pause <automation_id>`.',
  automation_resume: 'Resume an automation: `/automation_resume <automation_id>`.',
  automation_delete: 'Delete an automation: `/automation_delete <automation_id>`.',
  jobs: 'List automations. Compatibility alias for `/automations`.',
  job_remove: 'Delete an automation: `/job_remove <job_id>`.',
...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      '/jobs - List automations',
      '/job_remove <id> - Remove automation',
    ],
  },
  {
...
export async function runDesktopSlashCommand(input: string, context: CommandContext): Promise<DesktopSlashCommandResult> {
  ...
  const { apiBaseUrl, token, sessionId: rawSessionId, envFilePath } = context;
  const sessionId = String(rawSessionId || '').trim() || undefined;
  if (SESSION_REQUIRED_COMMANDS.has(command.name) && !sessionId) {
    return {
      handled: true,
      output: DESKTOP_NO_ACTIVE_SESSION_STATUS,
      status: 'select a chat',
    };
  }
  ...
  switch (command.name) {
    ...
    case 'reset':
      if (!['confirm', 'yes'].includes(normalize(command.args[0] || ''))) {
        return {
          handled: true,
          output: 'Reset clears the current session context and pending files. Run `/reset confirm` to continue.',
    ...
    case 'schedule': {
      ...
      return {
        handled: true,
        output: `Created automation ${job.name} (${job.id}) with "${job.schedule || schedule}".`,
        status: 'automation scheduled',
        refresh: true,
      };
    }
    ...
    case 'automation_delete': {
      if (!command.args.length) {
        return { handled: true, output: `Usage: /${command.name} <automation_id>` };
      }
      const action = command.name === 'automation_run'
        ? 'run'
        : command.name === 'automation_pause'
          ? 'disable'
          : command.name === 'automation_resume'
            ? 'enable'
            : 'delete';
      if (action === 'delete' && !['confirm', 'yes'].includes(normalize(command.args[1] || ''))) {
        return {
          handled: true,
          output: `Deleting automation ${command.args[0]} requires confirmation. Run \`/${command.name} ${command.args[0]} confirm\` to continue.`,
          status: 'confirmation needed',
        };
      }
      ...
      if (action === 'delete') {
        const confirmation = await createPendingConfirmation(apiBaseUrl, token, {
          ...
          message: `Delete automation ${command.args[0]} from the live scheduler.`,
          risk_tier: 'danger',
          origin_surface: 'desktop_command',
          payload: { automation_id: command.args[0], command: command.name },
        });
        const approved = await approvePendingConfirmation(apiBaseUrl, token, confirmation.confirmation_id, 'desktop_command');
        confirmationId = approved.confirmation_id;
      }
      await actOnJob(apiBaseUrl, token, command.args[0], action, confirmationId);
      return {
        handled: true,
        output: `${command.name.replace('automation_', '').replace('delete', 'deleted')} automation ${command.args[0]}.`,
    ...
    case 'job_remove': {
      ...
      if (!['confirm', 'yes'].includes(normalize(command.args[1] || ''))) {
        return {
          handled: true,
          output: `Deleting automation ${command.args[0]} requires confirmation. Run \`/job_remove ${command.args[0]} confirm\` to continue.`,
          status: 'confirmation needed',
        };
      }
      const confirmation = await createPendingConfirmation(apiBaseUrl, token, {
      ...
      const approved = await approvePendingConfirmation(apiBaseUrl, token, confirmation.confirmation_id, 'desktop_command');
      await actOnJob(apiBaseUrl, token, command.args[0], 'delete', approved.confirmation_id);
      return {
        handled: true,
        output: `Deleted automation ${command.args[0]}.`,
        status: 'automation removed',
        refresh: true,
      };
    }
