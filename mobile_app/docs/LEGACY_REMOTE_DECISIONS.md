# Legacy Kraitos Remote App Decisions

Status: archived. These decisions describe the old account-backed mobile
companion direction. The current product path is local-first desktop with no
account, VPS, cloud backend, or mobile pairing enabled by default.

## Archived product decisions
...
- Remote access in this archived path meant HTTPS, token-based auth, secure WebSocket auth, and VPS-friendly deployment.
