        ...
        with self._lock:
            ...
            if existing:
                self._conn.execute(
                    ...
                    """,
                    (time.time(), int(existing["user_id"])),
                )
                self._delete_unverified_signup_user_if_orphaned_locked(int(existing["user_id"]))
            ...
            challenge = self._create_auth_otp_challenge_locked(
                ...
                pending_password_hash=_hash_password(password, password_salt),
