...
from mobile_app.backend.fleet_policy import FLEET_PREVIEW_MODE, normalize_fleet_enrollment_ttl
...
class RemoteControlStoreFleetWorkerMixin:
        ...
        *,
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                """,
                (worker_id, user_id, desktop_id, instance_id, name, enrollment["metadata"] or "{}", now, now, now),
            )
            self._conn.execute(
                """
                INSERT INTO fleet_instances(
                ...
                """,
                (instance_id, user_id, desktop_id, worker_id, name, _json_dumps({"kind": "remote"}), now, now),
            )
    ...
    def delete_worker(self, *, user_id: int, worker_id: str, wipe_state: bool = True) -> Dict[str, Any]:
        with self._lock:
            ...
            self._conn.commit()
            self._secure_db_files()
            return {"ok": True, "deleted": True, "worker_id": worker["worker_id"], "wipe_state": bool(wipe_state)}
