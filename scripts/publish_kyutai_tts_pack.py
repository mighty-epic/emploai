...
DEFAULT_PACK_DIR = Path.home() / "Documents" / "Models" / "kyutai-pocket-tts-emploai-clone-pack"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Publish the runtime-ready Kyutai/Pocket speech pack to a Hugging Face model repo."
    )
    parser.add_argument("--repo-id", required=True, help="Destination Hugging Face repo id, for example org/model-name.")
    parser.add_argument("--pack-dir", default=str(DEFAULT_PACK_DIR), help="Local runtime-ready Kyutai pack directory.")
...
def main() -> int:
    ...
    required_paths = (
        pack_dir / TTS_VOICES_DIRNAME / DEFAULT_KYUTAI_TTS_VOICE_FILENAME,
        pack_dir / TTS_SITE_PACKAGES_DIRNAME / "pocket_tts",
    ...
    with tempfile.TemporaryDirectory(prefix="emploai-kyutai-pack-publish-") as temp_dir:
        ...
        if callable(upload_large_folder):
            ...
            api.upload_folder(
                ...
                repo_type="model",
                revision=args.revision,
                commit_message="Publish runtime-ready Kyutai TTS pack",
            )

    print(f"Published Kyutai TTS pack to https://huggingface.co/{args.repo_id}")
    return 0
