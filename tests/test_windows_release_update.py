...
def test_find_available_update_falls_back_to_github_when_manifest_fails(monkeypatch):
    ...
    monkeypatch.setattr(release_update.requests, "get", _fake_get)
...
def test_download_update_asset_verifies_sha256(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(release_update.requests, "get", lambda *args, **kwargs: _FakeStreamResponse(content))

    installer = release_update._download_update_asset(
        tmp_path,
        AvailableUpdate(
            version="0.1.0b2",
            tag_name="v0.1.0-beta.2",
...
def test_download_update_asset_rejects_sha256_mismatch(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="SHA-256"):
        release_update._download_update_asset(
            tmp_path,
            AvailableUpdate(
                version="0.1.0b2",
                tag_name="v0.1.0-beta.2",
...
def test_install_latest_update_passes_restart_executable(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(release_update, "find_available_update", lambda _info: update)
    ...
    monkeypatch.setattr(release_update, "install_available_update", _fake_install)
