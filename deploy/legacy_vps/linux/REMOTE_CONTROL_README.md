# Legacy EmploAI Remote Control Plane on Ubuntu 22.04

This is the preserved deployment path for the old **public mobile control plane**. It is kept for reference and possible future reconnection, but the current product path is the local-first desktop app with no EmploAI VPS or hosted domain.

Use this legacy path only when you intentionally want:
...
- FastAPI app from [remote_control_service.py](../../../app_backend/remote_control_service.py)
- `nginx` reverse proxy
...
## Files
- [bootstrap_remote_control.sh](bootstrap_remote_control.sh)
- [remote_control.env.example](remote_control.env.example)
- [emploai-remote-control.service](emploai-remote-control.service)
- [nginx-remote-control.conf](nginx-remote-control.conf)
...
```bash
cd /opt/emploai
sudo APP_USER=emploai APP_ROOT=/opt/emploai bash deploy/legacy_vps/linux/bootstrap_remote_control.sh
```

## Environment
```bash
sudo mkdir -p /etc/emploai
sudo cp deploy/legacy_vps/linux/remote_control.env.example /etc/emploai/remote-control.env
...
## systemd
```bash
sudo cp deploy/legacy_vps/linux/emploai-remote-control.service /etc/systemd/system/
...
## nginx
```bash
sudo cp deploy/legacy_vps/linux/nginx-remote-control.conf /etc/nginx/sites-available/emploai-remote-control
