...
def install_latest_update(home: Path, root: Path, *, restart_executable: Path | None = None) -> dict[str, Any]:
    from deploy.windows.release_update import install_latest_update as _install_latest_update
...
try:
    from deploy.windows import release_backend_bootstrap as _release_backend_bootstrap
    from deploy.windows import release_backend_cloud as _release_backend_cloud
    from deploy.windows import release_backend_records as _release_backend_records
    from deploy.windows import release_backend_services as _release_backend_services
except ImportError:
    import release_backend_bootstrap as _release_backend_bootstrap
    import release_backend_cloud as _release_backend_cloud
    import release_backend_records as _release_backend_records
    import release_backend_services as _release_backend_services

_RELEASE_BACKEND_MODULES = (
    _release_backend_cloud,
    _release_backend_records,
    _release_backend_services,
    _release_backend_bootstrap,
)
_RELEASE_BACKEND_CLOUD_NAMES = (
...
_RELEASE_BACKEND_SERVICE_NAMES = (
    ...
    '_is_runtime_process',
    '_is_release_backend_worker_process',
...
_RELEASE_BACKEND_BOOTSTRAP_NAMES = (
    '_bootstrap_payload',
...
_RELEASE_BACKEND_FUNCTION_NAMES = (
    *_RELEASE_BACKEND_CLOUD_NAMES,
    *_RELEASE_BACKEND_RECORD_NAMES,
    *_RELEASE_BACKEND_SERVICE_NAMES,
    *_RELEASE_BACKEND_BOOTSTRAP_NAMES,
)
for _release_backend_name in _RELEASE_BACKEND_FUNCTION_NAMES:
    for _release_backend_module in _RELEASE_BACKEND_MODULES:
        if hasattr(_release_backend_module, _release_backend_name):
            globals()[_release_backend_name] = getattr(_release_backend_module, _release_backend_name)
            break


def _release_backend_delegate(name: str):
    return lambda *args, __name=name, **kwargs: globals()[__name](*args, **kwargs)


def _sync_release_backend_modules() -> None:
    snapshot = dict(globals())
    skip_names = {
        '_release_backend_module',
        '_release_backend_name',
        '_RELEASE_BACKEND_MODULES',
    }
    for module in _RELEASE_BACKEND_MODULES:
        for name, value in snapshot.items():
            if name in skip_names:
                continue
            if callable(value) and not isinstance(value, type):
                setattr(module, name, _release_backend_delegate(name))
            else:
                setattr(module, name, value)


_sync_release_backend_modules()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "check-updates":
        from deploy.windows.release_update import check_for_updates
