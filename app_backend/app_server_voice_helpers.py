...
def _capture_runtime_status() -> Dict[str, Any]:
    ...
    from app_backend.capture_runtime import get_capture_runtime_status
...
def _voice_runtime_status() -> Dict[str, Any]:
    ...
    from app_backend.voice_runtime import get_voice_runtime_status
...
def _preload_tts_engine() -> Dict[str, Any]:
    ...
    from app_backend.voice_runtime import preload_tts_engine
...
def _configure_tts_backend(backend_value: str) -> dict[str, Any]:
    ...
    from app_backend.voice_runtime import (
    ...
    try:
        ...
        home = runtime_home()
        config_root = home if home is not None else _workspace_root()
        config = get_live_config(config_root / "config.json")
        config.set("voice.tts_backend", backend, user_id=0, note="selected from desktop voice settings")
        if backend == "kokoro_onnx":
            config.set("voice.tts_packs.kokoro_tts.requested", True, user_id=0, note="selected from desktop voice settings")
        elif backend == "pocket":
            config.set("voice.tts_packs.kyutai_clone_tts.requested", True, user_id=0, note="selected from desktop voice settings")
        config.set("voice.selection_source", "settings", user_id=0, note="selected from desktop voice settings")
        config.save_config()
    except Exception:
        logger.exception("[app] failed to persist TTS backend selection")
...
def _new_voice_draft_state():
    ...
    from app_backend.voice_runtime import new_voice_draft_state

    return new_voice_draft_state()

def _jarvis_fast_final_enabled() -> bool:

    from app_backend.voice_runtime import jarvis_fast_final_enabled

    return jarvis_fast_final_enabled()

def _jarvis_pending_wait_seconds() -> float:

    from app_backend.voice_runtime import jarvis_pending_wait_seconds

    return jarvis_pending_wait_seconds()

def _synthesize_assistant_audio_sync(text: str):

    from app_backend.voice_runtime import synthesize_assistant_audio
