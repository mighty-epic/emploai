...
class RemoteControlStoreSyncMixin:
    ...
    def create_pairing(self, *, user_id: int, desktop_id: str, ttl_seconds: int = REMOTE_PAIRING_TTL_SECONDS) -> Dict[str, Any]:
        with self._lock:
            ...
            self._conn.execute(
                """
                INSERT INTO pairings(pairing_id, pairing_token_hash, user_id, desktop_id, created_at, expires_at, used_at, revoked_at)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (pairing_id, _hash_token(pairing_token), int(user_id), desktop_id, now, now + ttl_seconds, None, None),
            )
            self._conn.commit()
    ...
    def complete_pairing(self, *, user_id: int, pairing_token: str, mobile_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            now = time.time()
            self._conn.execute("UPDATE mobiles SET paired_desktop_id = ?, last_used_at = ? WHERE mobile_id = ?", (desktop_id, now, mobile_id))
            self._conn.execute("UPDATE desktops SET paired_mobile_ids = ?, last_seen_at = ? WHERE desktop_id = ?", (_json_dumps(paired_mobile_ids), now, desktop_id))
            self._conn.execute("UPDATE pairings SET used_at = ? WHERE pairing_id = ?", (now, pairing["pairing_id"]))
            state = self._ensure_shared_state_locked(user_id)
            state["current_desktop_id"] = desktop_id
            state = self._bump_shared_state_locked(user_id, state)
            self._cleanup_locked()
            self._conn.commit()
            self._secure_db_files()
            desktop = self._conn.execute("SELECT * FROM desktops WHERE desktop_id = ?", (desktop_id,)).fetchone()
    ...
    def repair_mobile_pairing(self, *, user_id: int, mobile_id: str, desktop_id: str) -> Dict[str, Any]:
        with self._lock:
            desktop = self._conn.execute(
                "SELECT * FROM desktops WHERE desktop_id = ? AND user_id = ?",
                (desktop_id, int(user_id)),
            ...
            mobile = self._conn.execute(
                "SELECT * FROM mobiles WHERE mobile_id = ? AND user_id = ?",
                (mobile_id, int(user_id)),
            ).fetchone()
            if not desktop or not mobile:
                raise KeyError("Desktop or mobile device is unavailable")
            ...
            if mobile_id not in paired_mobile_ids:
                paired_mobile_ids.append(mobile_id)
            self._conn.execute(
                "UPDATE mobiles SET paired_desktop_id = ?, last_used_at = ? WHERE mobile_id = ?",
                (desktop_id, now, mobile_id),
            )
            self._conn.execute(
                "UPDATE desktops SET paired_mobile_ids = ?, last_seen_at = ? WHERE desktop_id = ?",
                (_json_dumps(paired_mobile_ids), now, desktop_id),
            )
